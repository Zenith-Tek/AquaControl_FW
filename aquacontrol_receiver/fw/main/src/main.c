...
void app_main() {
  ...
  if (supabase_is_online()) {
    ...
    ESP_LOGW(
        "MAIN",
        "Offline: skipping initial crash reporting and configuration fetch");
  }
  initialize_sntp();
  initialise_lora();
  xTaskCreate(&device_control_task, "Device Control Task", 16384, NULL, 6,
              NULL);
  xTaskCreate(&task_rx, "RX", 16384, NULL, 5, NULL);
  xTaskCreate(&supabase_heartbeat_task, "Supabase Heartbeat Task", 16384, NULL,
              3, &heartbeat_task_handle);
}
